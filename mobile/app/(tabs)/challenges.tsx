...
const CATS = [
  ...
  { id: 'daily',       label: '⭐ اليوم',        color: '#f59e0b' },
  { id: 'n8n',         label: '⚡ n8n',          color: '#CE82FF' },
  { id: 'ai_video',    label: '🎬 AI فيديو',     color: '#1CB0F6' },
  { id: 'vibe_coding', label: '💻 Vibe',         color: '#58CC02' },
  { id: 'general',     label: '📚 عام',          color: '#64748b' },
]
  ...
  const loadData = async () => {
    try {
      const [{ data: ch, error: chErr }, { data: att }] = await Promise.all([
        supabase.from('challenges').select('*').eq('is_active', true).is('linked_lesson_id', null)
        .order('category').order('sort_order').order('sort_order'),
  ...
  return (
    <SafeAreaView style={s.container} edges={['top']}>
      ...
      <ScrollView contentContainerStyle={s.scroll} showsVerticalScrollIndicator={false}>
        ...
        ) : (filterCat === 'all' ? challenges : challenges.filter((ch: any) => ch.category === filterCat)).map(ch => {
